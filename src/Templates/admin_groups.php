    ...
    <div class="container">
        ...
        <div class="section">
            ...
            <?php if (!empty($groups)): ?>
                    ...
                    <div class="group-card" data-group-id="<?php echo $group['id']; ?>">
                        ...
                        <ul class="group-users">
                            <?php if (!empty($usersInGroups[$group['id']])): ?>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <li class="user-item" style="background: #f8f9fa; text-align: center; color: #6c757d;">Нет пользователей в этой группе.</li>
                            <?php endif; ?>
                        </ul>
                        ...
                        <form class="add-user-form" method="post" action="/add-user-to-group">
                            ...
                            <select name="user_id" required style="flex-grow: 1;">
                                ...
                                <?php foreach ($allUsers as $user): ?>
                                    ...
                                    $isInGroup = false;
                                    if (!empty($usersInGroups[$group['id']])) {
                                        foreach ($usersInGroups[$group['id']] as $existingUser) {
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <div class="group-card" style="text-align: center; padding: 2rem; background: #f8f9fa;">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        if (e.target.classList.contains('save-btn')) {
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    if (input) input.remove(); else console.warn('Элемент .group-input не найден при сохранении.');
                    button.remove(); // Удаляем saveBtn
                    const cancelBtn = groupCard.querySelector('.cancel-btn'); // Нужно найти cancelBtn заново
                    if (cancelBtn) cancelBtn.remove(); else console.warn('Кнопка .cancel-btn не найдена при сохранении.');
    ...
    document.addEventListener('click', function(e) {
        if (e.target.classList.contains('cancel-btn')) {
            ...
            if (title) showElement(title); else console.error('Элемент .group-title не найден при отмене.');
            if (input) input.remove(); else console.warn('Элемент .group-input не найден при отмене.');

            button.remove(); // Удаляем cancelBtn
            const saveBtn = groupCard.querySelector('.save-btn'); // Нужно найти saveBtn заново
            if (saveBtn) saveBtn.remove(); else console.warn('Кнопка .save-btn не найдена при отмене.');
